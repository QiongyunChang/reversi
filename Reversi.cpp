...
#include <math.h>
#include <windows.h>
#include "mmsystem.h" //專案選項中&編輯器選項加入lwinmm  
...
void playsound_2(){
	PlaySound("son.wav",NULL,SND_FILENAME|SND_ASYNC);
}
...
void drawCircleChess(int row, int col, int color);
void chooseColor();
...
int PLAYONE_COLOR = WHITE; //設定玩家一顏色
int PLAYTWO_COLOR = BLACK; //設定玩家二顏色
...
void drawChessboard(int field[][GRID_NUM]){
   	...
   	int border_color = BROWN;
   	cleardevice(); //清理螢幕畫面 
	setfillstyle(SOLID_FILL,BROWN); 
   	floodfill(startX,startY,BROWN);
...
void setChess(int field[][GRID_NUM], Location chessLoc){
	...
	if(currentPlayer == PLAY_ONE){
		drawCircleChess(chessLoc.row, chessLoc.col, PLAYONE_COLOR);
		play1Score++;
		if(field[chessLoc.row][chessLoc.col]==PLAY_TWO)
			play2Score--;
	}else{
		drawCircleChess(chessLoc.row, chessLoc.col, PLAYTWO_COLOR);
		play2Score++;
...
void chooseColor(){
   	cleardevice(); //清理所有螢幕資料，如果希望只顯示訊息時，取消註解 
	int i = 0;
	char msg0[50] = "Press [p] to start ";
	char msg01[50] = "Press [q] to close ";
	char msg1[50] = "You can hoose color ：";
	char msg2[100] = "[a] Yellow & TiffianyGreen";
	char msg3[100] = "[b] Black & WHite";
   	char msg4[100] = "[c] Pink & Purple";
	   
	for(; ; i++){
		setcolor(i%14);
	   	settextstyle(BOLD_FONT, HORIZ_DIR , 5);
	   	outtextxy(0, SCREEN_HEIGHT / 2, msg0);
		
		setcolor(i%14+1);
	   	settextstyle(BOLD_FONT, HORIZ_DIR , 4);
	   	outtextxy(0, SCREEN_HEIGHT / 2+50, msg01);
		
	   	setcolor(i%14+2);
	   	settextstyle(BOLD_FONT, HORIZ_DIR , 3);
	   	outtextxy(0, SCREEN_HEIGHT / 2+100, msg1);
	   	
	   	setcolor(WHITE);
	   	settextstyle(COMPLEX_FONT, HORIZ_DIR , 1);
	   	outtextxy(20, SCREEN_HEIGHT / 2 + 150, msg2);
	   	
	   	setcolor(WHITE);
	   	settextstyle(COMPLEX_FONT, HORIZ_DIR , 1);
	   	outtextxy(20, SCREEN_HEIGHT / 2 + 200, msg3);
		...
		setcolor(BLACK);
	   	...
	   	if(kbhit()) {
			...
			if (colorkey == 'a' || colorkey == 'A') {
				PLAYONE_COLOR = YELLOW; //設定玩家一顏色
				PLAYTWO_COLOR = 3;
				return; 				
			}
			else if(colorkey == 'b' || colorkey == 'B'){
				PLAYONE_COLOR = WHITE; //設定玩家一顏色
				PLAYTWO_COLOR = BLACK;
				return;
			}
			else if(colorkey == 'c' || colorkey == 'C'){
				PLAYONE_COLOR = 5; //設定玩家一顏色
				PLAYTWO_COLOR = 9;
				return;
			}
			else if (colorkey == 'q' || colorkey == 'Q'){
				closeGame(); //如果玩家輸入'q'離開遊戲	
			}
			else if(colorkey == 'p' || colorkey == 'P'){
				return;
			}
		}
	}	
